...
def categoriess(request):
    items=Listing.objects.raw("SELECT * FROM auctions_listing GROUP BY categorys")
    try:
        w = Watchlist.objects.filter(user=request.user.username)
        wcount=len(w)
    except:
        wcount=None
    return render(request,"auctions/categpages.html",{
        "items": items,
        "wcount":wcount
    })

def categoriesss(request):
    items=Listing.objects.raw("SELECT * FROM auctions_listing GROUP BY categoryss")
    try:
        w = Watchlist.objects.filter(user=request.user.username)
        wcount=len(w)
    except:
        wcount=None
    return render(request,"auctions/categpagess.html",{
        "items": items,
        "wcount":wcount
    })            
...
def categoryss(request,categoryss):
    ...
    try:
        ...
        wcount=None
    return render(request,"auctions/categoryss.html",{
        "items":catitemsss,
        "catss":categoryss,
        "wcount":wcount
    })
...
def submit(request):
    if request.method == "POST":
        ...
        if request.POST.get('video'):
            listtable.video = request.POST.get('video')     
        else : 
            listtable.video = "https://www.youtube.com/embed/dQw4w9WgXcQ"      
        listtable.time = dt
        listtable.save()
        all = Alllisting()
        items = Listing.objects.all()
